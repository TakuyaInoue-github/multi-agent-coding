...
`/loop` で起動している場合、毎サイクルで以下を順に確認し、該当するアクションを実行する。

### チェック順序

1. **自分が担当中のタスクが完了しているか**
   - `.assigned` ファイルに自分の worker-id があるタスクを探す
   - `result.md` が未作成なら Codex の進捗を確認（`/codex:status`）
   - 完了していれば `result.md` を作成し `.assigned` を削除
   - blocked なら `result.md` に `status: blocked` で記録し `.assigned` を削除

2. **担当タスクがない場合、新規タスクを取得できるか**
   - BOARD.md で `status: approved` かつ `.assigned` が存在しないタスクを探す
   - 候補が複数ある場合は task-id の昇順で最初の1件を取得
   - `.assigned` ファイルを作成（中身: `worker_id: <自分のID>`, `acquired_at: <timestamp>`）
   - 読み返して自分の worker-id が入っていることを確認（他の Worker との競合チェック）
   - 確認できたら実装を開始（`task/task-xxx` ブランチ作成 → Codex に委譲）
...
起動時に以下をセッション内で宣言し、以後一貫して使用する：

```
worker_id: worker-1   # tmux セッション名に合わせる
```

### ループ内での行動原則

- 1サイクルで担当できるタスクは1件のみ（並列実装は行わない）
- Codex がバックグラウンド実行中のサイクルは「完了確認のみ」を行い、新規取得はしない
- blocked または failed のタスクは `.assigned` を削除して次のサイクルで新規取得に戻る
